...
-- Converted for PostgreSQL 17: TINYINT→SMALLINT, CHARACTER LARGE OBJECT→TEXT,
--   BINARY VARYING(255)→BYTEA, ENUM(...)→VARCHAR(255),
--   BIGINT GENERATED BY DEFAULT AS IDENTITY kept (valid PG syntax),
--   H2 CHECK syntax on quoted identifiers updated to unquoted.
...
-- ENUM('ADMIN','SUPERADMIN','USER') → VARCHAR(255)
-- PostgreSQL supports native ENUM types but VARCHAR is simpler for Flyway migrations
-- and avoids needing a separate CREATE TYPE statement that Hibernate doesn't manage.
...
-- BIGINT GENERATED BY DEFAULT AS IDENTITY is valid PostgreSQL syntax (SQL standard).
